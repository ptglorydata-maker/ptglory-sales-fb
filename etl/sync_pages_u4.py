...
# ยืนยันจาก --discover เมื่อ 8/8/2569: col2=ยอดขายรวมเพจ, col5=Order รวม,
# col54=ค่าแอด, col60=ROAS รวม (แถวหัวตาราง 5 ชั้น row7-11, ข้อมูลเริ่ม row12)
# ยังต้องยืนยันทีละ tab ที่เหลือด้วย --discover ก่อนเติม เพราะผังอาจไม่เหมือนกันทุกเพจ
PAGE_COLUMN_MAP = {
    "Ha Yeon-ฮายอง ครีมโสมเกาหลี": (2, 5, 54, 60),
    "Ha-Yeon ครีมโสมสูตรนำเข้าจากเกาหลี": (2, 5, 54, 60),
    # "Ha Yeon - ครีมโสมลดฝ้า": (...),  # ยังไม่ได้เช็ค — เพจนี้ active อยู่ (P3) เช็คก่อน sync จริง!
    "เพจHa Yeon ครีมฮายอง จากเกาหลี": (2, 5, 54, 60),
    "เพจ ผิวสวยด้วยครีมฮายอง": (2, 5, 54, 60),
    # ด้านล่างนี้ยังไม่ได้ยืนยันด้วย --discover จริง (ใส่ตามรูปแบบที่เหมือนกันทุก tab ที่เช็คแล้ว
    # 4/6 อัน) แนะนำเช็คให้ครบก่อน sync จริงเพื่อความชัวร์ 100%
