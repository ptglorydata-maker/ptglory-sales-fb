...
// TTL_RESULT เดิม = 300 วิ เท่ากับรอบ cron อุ่น cache (*/5 * * * *, ดู wrangler.toml) พอดิบพอดี — ถ้ารอบ
// cron รอบใดรอบหนึ่งช้า/พลาด (เช่น Google Sheets API ตอบช้าตอนนั้นพอดี ซึ่ง warmDefaultCaches_() ดักด้วย
// .catch เงียบๆ ไม่ report error) cache จะหมดอายุพอดีก่อนรอบถัดไปมาอุ่นทัน ผู้ใช้ที่เข้ามาในโพรงว่างนั้นเลย
// ต้องรอคำนวณสด (อ่าน Google Sheets หลายชีตพร้อมกัน) ซึ่งบางครั้งช้าเกิน timeout 25 วิฝั่งหน้าเว็บ — ยืดเป็น
// 3 เท่าของรอบ cron ให้มี margin กันรอบ cron พลาดไป 1-2 ครั้งติดก็ยังไม่กระทบผู้ใช้จริง
const TTL_RESULT = 900;       // ผลลัพธ์ที่คำนวณเสร็จแล้วต่อชุดพารามิเตอร์
